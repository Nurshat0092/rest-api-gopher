(function () {
    ...
    i18n.create = function (data, current) {
        var trans;
        if (!!current) {
            if (!!current.data) {
                var lang_keys = Object.keys(current.data.values)
                for (var i = 0; i < lang_keys.length; i++) {
                    data.values[lang_keys[i]] = current.data.values[lang_keys[i]];
                }
            }
        }
